...
DS18xx::~DS18xx(){/*destrutor do construto*/}

DS18xx::DS18xx(uint8_t pin) : // métrodos definos como erança do onewire wire-1
...
uint8_t DS18xx::select(uint8_t address[]) {
...
String DS18xx::ObterNumeroDespositos(){
...
uint8_t DS18xx::selectNextAlarm() {
...
void DS18xx::resetSearch() {
    lastDiscrepancy = 0;
    lastDevice = 0;
}

float DS18xx::ObterTemperaturaCelcius() {
...
float DS18xx::ObterTemperaturaFahrenheit() {
  return ObterTemperaturaCelcius() * 1.8 + 32;
}

uint8_t DS18xx::ObterResolucao() {
...
void DS18xx::setResolution(uint8_t resolucao) {
...
uint8_t DS18xx::ObterModoPower(){
    return selectedPowerMode;
}//metodo que rtorna uma string 

String DS18xx::ObterModoPowerString(){
...
uint8_t DS18xx::ObterCodigoFamilia(){
    return selectedAddress[0];
}

 
String  DS18xx::ObterCodigoFamiliaString() {
...
void DS18xx::ObterEnderecoFisico(uint8_t Endereco[]) {
    memcpy(Endereco, selectedAddress, 8);//copia o vetor para o outro 
}
uint8_t* DS18xx::ArrayObterEnderecoFisico(uint8_t Endereco[]) {
    memcpy(Endereco, selectedAddress, 8);//copia o vetor para o outro 
   return Endereco;
}
String  DS18xx::DecimalObterEnderecoFisico(uint8_t Endereco[]) {
  String Array="";
    memcpy(Endereco, selectedAddress, 8);//copia o vetor para o outro 
   for(int i=0; i <8;i++){ 
     Array+=" ";
...
void  DS18xx::PrintObterEnderecoFisico(uint8_t Endereco[]) {
    memcpy(Endereco, selectedAddress, 8);//copia o vetor para o outro 
...
void DS18xx::getAddress(uint8_t address[]) {
    memcpy(address, selectedAddress, 8);//copia o vetor para o outro 
}


void DS18xx::TConvercao() {
...
uint8_t DS18xx::sendCommand(uint8_t romCommand) {
    if (!umfio.reset()) {
        return 0;
    }
...
uint8_t DS18xx::sendCommand(uint8_t romCommand, uint8_t functionCommand, uint8_t power) {
...
uint8_t DS18xx::oneWireSearch(uint8_t romCommand) {
...
uint8_t DS18xx::isConnected(uint8_t address[]) {
...
void DS18xx::delayForConversion(uint8_t resolucao, uint8_t powerMode) {
    if (powerMode) {
